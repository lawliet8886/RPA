# RPA Organizador (Android) – v0.3.1
...
## O que já está implementado (v0.3.1)
...
## CI no GitHub Actions (gera o APK em Artefatos)

Este repositório já vem com um workflow em `.github/workflows/android-debug-apk.yml` que:

- instala Java 17
- instala Android SDK (platform 35 + build-tools 35.0.0)
- roda `./gradlew :app:assembleDebug` com `--stacktrace` e `--info`
- sobe o `app-debug.apk` e o `gradle-build.log` em **Artifacts**

Se der erro, o log do Gradle vai estar nos artefatos para você mandar aqui e eu corrigir.
